...
The `ascender-deployment` file will remove the Ascender installation. The last command will remove the ascender-operator deployment.
...
### Removing Namespaces
Another way to completely remove Ascender is by deleting its namespace.

- ***This will delete all resources in the namespace, including PVCs, without prompting.  So be sure you are deleting the proper namespace and that only Ascender lives in that namespace***
