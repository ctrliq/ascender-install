...
if [[ ! -f "${CONFDIR}/certs/ascender.crt" || ! -f "${CONFDIR}/certs/ascender.key" ]]; then
    ...
    echo "  (replace ascender.crt / ascender.key with your own for a real certificate, then re-run install.sh)"
fi
# Always, so a certificate the operator copied in gets the same treatment as a
# generated one: nginx runs as uid 1000, gid 0 inside the container, so the
# group (0) gets read access while the private key stays out of reach of other
# local users. A copied key typically arrives root:root 0600, which nginx
# cannot open.
